...
var SettingsQuestions = ( {currentUser, history} ) => {

  // get application data from sessionStorage
  const application = JSON.parse(sessionStorage.getItem('application'));

  // if sessionStorage is empty, redirect to /home
  if (application===null) {window.location="/home";}
  ...
  return (
    <div>
      <div>
        ...
        <fieldset>
          {/* Form Name */}
          <legend>My visa application </legend>
          {/* Text input*/}
          <div className="form-group">
            <label className=" control-label" htmlFor="applicantName">Name of applicant</label>  
            <div className="">
...
{/* Multiple Checkboxes */}
          ...
          <div className="form-group">
            ...
            <Button
              ...
              {' '}
              <button id="singlebutton" name="singlebutton" className="btn btn-primary">Save</button> 
            </div>
          </div>
        </fieldset>
      </Form>
...
<div className="d-flex justify-content-center mb-4 pt-0 pb-4">
<Card style={{ width: '28rem' }} id="settings">
  <Card.Header>Account info</Card.Header>
  <ListGroup variant="flush">
    <ListGroup.Item><div>Name: {currentUser.displayName}</div>
    <div>Email: {currentUser.email}</div>
    </ListGroup.Item>
    <ListGroup.Item><Button variant="light" onClick={() => waitAndSignOut()}>
    Sign out
